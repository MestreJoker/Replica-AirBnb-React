...
const Navigation = () => {
    const iconesMenu = icones.map((item) => (
        <SwiperSlide key={item.id} className='p-1 h-full iconeMenu'>
            <div className='flex justify-center items-center'>
                <img src={item.source} alt={`Imagem ${item.description}`} className='w-6 h-6' />
            </div>
    ...
    return (
        <nav className='pt-6 flex items-center gap-3 h-15'>
            <Swiper id='iconesMenu'
                spaceBetween={15}
                slidesPerView={2.5} // Valor padrão para mobile
                onSlideChange={() => console.log('slide change')}
                onSwiper={(swiper) => console.log(swiper)}
                breakpoints={{
                    // Correção dos valores dos breakpoints
                    640: {
                        slidesPerView: 2.5
                    },
                    768: { // Corrigido: 768 em vez de 764
                        slidesPerView: 4.5
                    },
                    1024: {
                        slidesPerView: 5.5
            ...
            <div className='pb-6'>
                <BotaoEntrar icone={(<IconAdjustmentsHorizontal />)}>
                    Filtro
                </BotaoEntrar>
            </div>
